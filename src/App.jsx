...
def _grid_db_insert_order(conn, wallet_address: str, item_id: str, order: dict, chain: str = "") -> str:
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT OR REPLACE INTO grid_orders(order_id,wallet_address,item_id,chain,side,price,qty,status,level,meta_json,created_ts,updated_ts) "
        "VALUES(?,?,?,?,?,?,?,?,?,?,COALESCE((SELECT created_ts FROM grid_orders WHERE order_id=?),?),?)",
...
def api_grid_order_stop():
    ...
    if isinstance(sess, dict):
        ...
        def _match(o: dict) -> bool:
            ...
            if o.get("status") != "OPEN": return False
            if oid is not None and str(oid).strip() != "":
                return str(o.get("id")) == str(oid)
            if side is None or level is None: return False
            if str(o.get("side")) != str(side): return False
            if o.get("level") != level: return False
...
# Persist to SQLite (authoritative). This makes orders survive refresh / other devices.
try:
    conn = _db()
    chain = str((payload.get("chain") if isinstance(payload, dict) else "") or "").strip()
    with DB_WRITE_LOCK:
        _grid_db_insert_order(conn, wa, item_id, order, chain=chain)
        # Ensure vault row exists; initialize from current session budget if available
        vt = _grid_db_vault_total(conn, wa, item_id, chain=chain)
        if vt <= 0:
            candidate = sess.get("vault_total") or sess.get("initial_capital_usd") or sess.get("initial_capital") or 0
            try:
                candidate = float(candidate)
            except Exception:
                candidate = 0.0
            if candidate > 0:
                _grid_db_set_vault_total(conn, wa, item_id, candidate, chain=chain)
        conn.commit()
except Exception as e:
    print("[WARN] grid manual add DB persist failed:", e)
finally:
    try:
        conn.close()
    except Exception:
