...
setGridMeta((prev) => ({ ...prev, ...getGridMetaFromResponse(r, { ...prev, gridItemId }) }));
  ...
  async function deleteGridOrder(orderId) {
    ...
    // Optimistic UI: hide immediately + persist removal so deleted orders do not resurrect
    ...
    try {
      ...
      if (nextOrdersOptimistic.length) {
        lastNonEmptyOrdersRef.current = { ts: Date.now(), count: nextOrdersOptimistic.length };
      } else {
      ...
      try {
        ...
        if (a.method === "DELETE") {
          ...
          if (Array.isArray(delOrdersRaw)) {
            ...
            try { rememberGridOrders(gridItemId, delOrders); } catch {}
          }
          setGridVaultStats((prev) => getGridVaultStatsFromResponse(r, prev));
          setGridMeta((prev) => ({ ...prev, ...getGridMetaFromResponse(r, { ...prev, gridItemId }) }));
          kickGridRefresh();
          ...
          if (Array.isArray(respOrdersRaw)) {
            ...
            try { rememberGridOrders(gridItemId, respOrders); } catch {}
          } else {
    ...
    try { rememberGridOrders(gridItemId, prevOrders || []); } catch {}
