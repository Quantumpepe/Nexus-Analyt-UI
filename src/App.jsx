...
setGridMeta((prev) => ({ ...prev, ...getGridMetaFromResponse(r, { ...prev, gridItemId }) }));
  ...
  async function deleteGridOrder(orderId) {
    ...
    // Optimistic UI: hide immediately and clear stale cached order state too
    const prevOrders = gridOrders;
    const cleanedOrders = (gridOrders || []).filter((o) => String(idOf(o)) !== String(orderId));
    setGridOrders(cleanedOrders);
    rememberGridOrders(gridItemId, cleanedOrders);
    try {
      gridOrdersCacheRef.current[gridItemId] = {
        ts: Date.now(),
        orders: cleanedOrders,
        lastNonEmptyOrders: cleanedOrders,
        lastNonEmptyTs: cleanedOrders.length ? Date.now() : 0,
      };
    } catch {}
